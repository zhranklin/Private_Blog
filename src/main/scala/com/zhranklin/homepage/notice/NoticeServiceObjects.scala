...
import com.zhranklin.homepage.notice.NoticeFetcher._
import org.jsoup.Jsoup
import org.jsoup.nodes.Document

object NoticeServiceObjects {
  class CSService(name: String, part: String, tpl: String) extends NoticeService(name,
    new SelectorUrlService(s".*/$part/.*\\d{10,}.*"),new IndexService(s"http://$tpl<index>.htm"),
    new FunNoticeFetcher(contentF("#BodyLabel"), dateF("td[width]:matches(时间.*来源)")))
  class LawService(title: String, listId: String) extends NoticeService(s"法学院 - $title",
    new AbstractUrlService {
      import org.json4s._
      def extractFromRawUrl(a: E) = ???; def rawUrlsFromDoc(doc: Document) = ???
  ...
  val serviceList = List(
    new NoticeService("教务处 - 通知", new SelectorUrlService("newsShow.*"),
      new IndexService("http://jwc.scu.edu.cn/jwc/moreNotice.action?url=moreNotice.action&type=2&keyWord=&pager.pageNow=<index>"),
      new FunNoticeFetcher(selectorF("input[name=news.content]")(_.first.attr("value")), dateF("table[width=900] td:contains(发布时间)"))),
    ...
    new NoticeService("电气信息学院 - 学院通知", new SelectorUrlService(".*detail\\.jsp"), new IndexService("http://seei.scu.edu.cn/student,p<index>,index.jsp"),
...
//    new FunNoticeFetcher(contentF("td[width=796]"), dateF("td:contains(发布时间)"))),
    new NoticeService("数学学院 - 学院新闻", new SelectorUrlService("News_show\\.asp\\?classid.*"), new IndexService( "http://math.scu.edu.cn/news.asp?PAGE=<index>"),
      new FunNoticeFetcher(contentF("td[height=504]"), dateF("td:contains(更新时间)"))),
    new NoticeService("经济学院 - 学院新闻", new SelectorUrlService("/news/\\d+.html"), new IndexService( "http://sesu.scu.edu.cn/news/list_1_<index>.html"),
      new FunNoticeFetcher(contentF("div.mt20"), dateF("div.info"))),
    new NoticeService("经济学院 - 学院公告", new SelectorUrlService("/gonggao/\\d+.html"), new IndexService( "http://sesu.scu.edu.cn/gonggao/list_2_<index>.html"),
      new FunNoticeFetcher(contentF("div.mt20"), dateF("div.info"))),
    new LawService("学院新闻", "8572"), new LawService("学院公告", "8573"),
    new NoticeService("文学与新闻学院 - 学院动态", new SelectorUrlService("ReadNews\\.asp"), new IndexService( "http://www.sculj.cn/Special_News.asp?SpecialID=40&SpecialName=%D1%A7%D4%BA%B6%AF%CC%AC&page=<index>"),
      new FunNoticeFetcher(contentF("tr:has(td[height=50]:matches(阅读\\d+次)) + tr > td"), dateF("td[height=50]:matches(阅读\\d+次)"))),
    new NoticeService("外国语学院 - 学院公告", new SelectorUrlService("/foreign/a/xueyuangonggao/\\d+/\\d+/\\d+\\.html"), new IndexService( "http://flc2.scu.edu.cn/foreign/a/xueyuangonggao/list_27_<index>.html"),
